...
const Form = () => {
    ...
    const {
        ...
        isStartingDrug, setIsStartingDrug,
        isStimulationDrug, setIsStimulationDrug,
        isBlastocyst, setIsBlastocyst,
        isAdjuvants, setIsAdjuvants
    ...
    const handleChangeProtocolType = (e) => {
        ...
        if (cycleTypeVal === "EggFreezing") {
            if (e.target.value.length > 0 && stimulationDrug.length > 0 && startingDose.length > 0) {
                setIsDisabled(false);
            } else {
                setIsDisabled(true);
            }
        } else if (cycleTypeVal === "EggThaw") {
            if (e.target.value.length > 0 && blastocystCulture.length > 0 && cycleTypeVal.length > 0) {
                setIsDisabled(false);
            } else {
                setIsDisabled(true);
            }
            ...
            if (e.target.value.length > 0 && stimulationDrug.length > 0 && startingDose.length > 0 && blastocystCulture.length > 0 && cycleTypeVal.length > 0) {
                setIsDisabled(false);
            } else {
                setIsDisabled(true);
            }
    ...
    const handleChangeStimulationDrug = (e) => {
        ...
        if (cycleTypeVal === "EggFreezing") {
            if (e.target.value.length > 0 && startingDose.length > 0 && protocolType.length > 0 && cycleTypeVal.length > 0) {
                setIsDisabled(false);
            } else {
                setIsDisabled(true);
            }
        } else if (e.target.value.length > 0 && protocolType.length > 0 && startingDose.length > 0 && blastocystCulture.length > 0 && cycleTypeVal.length > 0) {
            setIsDisabled(false);
        } else {
            setIsDisabled(true);
        }
    ...
    const handleChangeStartingDose = (e) => {
        ...
        if (cycleTypeVal === "EggFreezing") {
            if (e.target.value.length > 0 && stimulationDrug.length > 0 && protocolType.length > 0 && cycleTypeVal.length > 0) {
                setIsDisabled(false);
            } else {
                setIsDisabled(true);
            }
        } else if (e.target.value.length > 0 && protocolType.length > 0 && stimulationDrug.length > 0 && blastocystCulture.length > 0 && cycleTypeVal.length > 0) {
            setIsDisabled(false);
        } else {
            setIsDisabled(true);
        }
    ...
    const handleChangeBlastocystCulture = (e) => {
        ...
        } else if (e.target.value.length > 0 && protocolType.length > 0 && stimulationDrug.length > 0 && startingDose.length > 0 && cycleTypeVal.length > 0) {
            setIsDisabled(false);
        } else {
            setIsDisabled(true);
        }
    ...
    const handleChangeAdjuvants = (position) => {
        ...
        updateChecketState.forEach((item, index) => {
            if (item === true) {
                ...
                valuesLabels.push(costData.Adjuvants[Object.keys(costData.Adjuvants)[index]].Value);
    ...
    const handleSubmit = (e) => {
        ...
        // figure costs
        const cycleTypeCostCalc = cycleTypeVal.length > 0 ? parseFloat(costData['CycleType'][cycleTypeVal].Cost) : 0;
        setCycleTypeCost(cycleTypeCostCalc);
        // console.log("cycle cost", cycleTypeCostCalc);
        const protocolTypeCostCalc = protocolType.length > 0 ? parseFloat(costData['ProtocolType'][protocolType].Cost) : 0;
        setProtocolTypeCost(protocolTypeCostCalc);
        // added 16/02 feedback from James
        const additionalProtocolDrugCost = protocolType.length > 0 ? parseFloat(costData['ProtocolType'][protocolType].AdditionalDrugCost) : 0;
        //console.log("additional protocol drug cost", additionalProtocolDrugCost);
        const blastocystCultureCostCalc = blastocystCulture.length > 0 ? parseFloat(costData['BlastocystCulture'][blastocystCulture].Cost) : 0;
        ...
        if (stimulationDrug.length > 0 && protocolType.length > 0 && startingDose.length > 0) {
            try {
                cyclePrice = parseFloat(costData['CyclePrice'][stimulationDrug + protocolType + startingDose].Cost);
                // figure drug text
                selectedDrugValue = costData['CyclePrice'][stimulationDrug + protocolType + startingDose].Value;
        ...
        setCycleCost(cyclePrice + additionalProtocolDrugCost);
        setSelectedDrugValue(selectedDrugValue);

        let totalCalculation = cyclePrice + cycleTypeCostCalc + protocolTypeCostCalc + blastocystCultureCostCalc + adjuvantsCostCalc + additionalProtocolDrugCost;
        ...
        if (cycleTypeVal.length > 0) {
            cycleTypeAdditionalDrugNotes = costData['CycleType'][cycleTypeVal].AdditionalDrugNotes;
        ...
        if (blastocystCulture.length > 0) {
            blastocystCultureNotes = costData['BlastocystCulture'][blastocystCulture].Notes;
        ...
        <div className="ivf-calculator-form">
            {
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <p className="ivf-calculator-intro">
                        {calculatorHeading}
                    </p>
                    {/* Cycle type select */}
                    <Select
                    ...
                    {/* Cycle type select */}
                    {
                        ...
                        <div className="ivf-calculator-input">
                            <div className="ivf-calculator-input__select">
                                <label className="ivf-calculator-input__select-label" htmlFor="protocolType">{protocolTypeLabel}</label>
                                <div className="ivf-calculator-input__select-container">
                                    ...
                                    >
                                        ...
                                        {
                                            (cycleTypeVal === "EggThaw" || cycleTypeVal === "EmbryoThaw") &&
                                            Object.keys(costData.ProtocolType).filter(item => item !== "Long" && item !== "Antagonist").map(function (key) {
                                                return (
                                                    <option key={uuid()} value={key}>{costData.ProtocolType[key].Value}</option>
                                                )
                                            })
                                        }
                                        ...
                                        {
                                            cycleTypeVal !== "EggThaw" && cycleTypeVal !== "EmbryoThaw" &&
                                            Object.keys(costData.ProtocolType).filter(item => item !== "MedicatedThawCycle" && item !== "NaturalThawCycle").map(function (key) {
                                                return (
                                                    <option key={uuid()} value={key}>{costData.ProtocolType[key].Value}</option>
                                                )
                                            })
                    ...
                    {/* Adjuvants checkboxes */}
                    {
                        isAdjuvants &&
                        <div>
                            <label htmlFor="adjuvants" className="ivf-calculator-checkbox__group-label">{adjuvantsLabel}</label><br></br>
                            {
                    ...
                    <br></br>
                    <button
                        className={`ivf-calculator__btn ${isDisabled ? "ivf-calculator__btn--disabled" : ""}`}

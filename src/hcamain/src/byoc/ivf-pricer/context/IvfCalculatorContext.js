...
const IvfCalculatorContextProvider = ({ children }) => {
    ...
    const [cycleTypeAdditionalDrugNotes, setCycleTypeAdditionalDrugNotes] = useState('');
    ...
    useEffect(() => {
        ...
        axios.get(`${urlHost}/lookupApi/IVFCostCalculator/default/findbydictionaryasobject/Costs`)
            .then(response => {
                ...
                let test = new Array(Object.keys(response.data.Adjuvants).length).fill(false);
                setCheckStatus(test);
                setIntroPDF(response.data?.Introduction?.Introduction?.Notes || '');
                setCalculatorHeading(response.data?.CalculatorHeading?.CalculatorHeading?.Notes || '');
                setCycleTypeLabel(response.data?.Label?.CycleType?.Label || '');
                setBlastocystCultureLabel(response.data?.Label?.BlastocystCulture?.Label || '');
    ...
    return (
        <IvfCalculatorContext.Provider
            value={{
                ...
                cycleNotes, setCycleNotes,
                cycleTypeAdditionalDrugNotes, setCycleTypeAdditionalDrugNotes,
                blastocystCultureNotes, setBlastocystCultureNotes,
                adjuvantsNotes, setAdjuvantsNotes,
                selectedDrugNotes, setSelectedDrugNotes,
                cycleCost, setCycleCost,
                selectedDrugValue, setSelectedDrugValue,
                adjuvantsValuesPrint, setAdjuvantsValuesPrint,
                introPDF,
                calculatorHeading,
                cycleTypeLabel,

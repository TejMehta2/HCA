...
const builtInCssClasses: Readonly<ResultsCountCssClasses> = {
  resultsCountContainer:
    'font-semibold text-neutral mb-4 py-2 mr-2.5 whitespace-nowrap',
...
function useResultsCount() {
  const isVertical =
    useSearchState((state) => state.meta.searchType) ===
    SearchTypeEnum.Vertical;
  const results = useSearchState((state) =>
    isVertical ? state.vertical : state.universal.verticals
  );
  ...
  if (results) {
    if (isUniversalSearchResults(results)) {
      results.forEach(
        (resultsOfAVertical) =>
          (resultsCount += resultsOfAVertical.resultsCount)
      );
    } else {
  ...
  if (resultsCount > limit && isVertical) {
    ...
    const resultCountWithPaginationText = `${paginateRange} of ${resultsCount}`;
...
function isUniversalSearchResults(
  data: VerticalResults[] | VerticalSearchState
): data is VerticalResults[] {

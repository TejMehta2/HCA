...
export type TbcBookingConfirmationClientProps = Omit<
  TbcBookingConfirmationProps,
  'componentMap'
> & {
...
type EcommerceDataLayerProps = Pick<
  TbcBookingConfirmationProps,
  'amount' | 'serviceName' | 'location' | 'extras' | 'type'
>;

const getStringValue = (value?: string | null): string => value || '';

function getEcommerceDataLayer(
  props: Partial<EcommerceDataLayerProps>,
  ...
  try {
    ...
    const rawValue = props.amount.replace(/[^\d.]/g, '');
...
const DefaultContent = (
            ...
            <Button size={'large'} variation={'full'}>
              <JssLink
                href={`${props.fields.ErrorCTALink?.value.href}${
                  confirmationProps.retryQuerystring || ''
                }`}
  ...
  const date = new Date(getStringValue(confirmationProps.appointmentDateTime));
  ...
  if (confirmationProps.extras) {
    options.splice(1, 0, {
      title: 'Extras',
      text: confirmationProps.extras,
    });
  }

  if (confirmationProps.amount) {
    options.push({
      title: props.fields?.AmountLabel?.value || 'Price paid',
      text: confirmationProps.amount,
    });
  }
  ...
  return (
    <>
      ...
      <FormContainer
        ...
        aside={<CFAside>{props.asidePlaceholder}</CFAside>}
      >
        <>
          <RichText>

...
export interface StepConsultantProfileServerSideProps {
  Slug: string;
...
export type StepConsultantProfileProps = {
  rendering: ComponentRendering;
  params: { [key: string]: string };
  fields: StepConsultantProfileFields;
};

const StepDefaultComponent = (
  props: StepConsultantProfileProps
): JSX.Element => (
...
export const Default = (props: StepConsultantProfileProps): JSX.Element => {
  ...
  const serverSideData = useComponentProps<StepConsultantProfileServerSideProps>(
    props.rendering.uid
  );
  ...
  // top specialty
  const topSpecialty =
    serverSideData?.ProfileJson?.keywords?.filter(
      (item: any) => item.parentName === 'ABSTRACT_TOP_LEVEL_KEYWORD'
    ) || [];
  const primarySpecialty = topSpecialty[0];
  ...
  const title = `${name} - ${primarySpecialty?.name || ''} at HCA Healthcare UK`;
  ...
  useEffect(() => {
    ...
    if (!doctifyLoaded) {
      ...
      window.dataLayer.push({
        ...
        consultantSpecialty: primarySpecialty?.name || '',
        consultantReviews:
          ...
          <div>
                  ...
                  <TextLink>
                    <Link
                      href={`${props?.fields?.ResultsLink?.value?.href
                        }?search=${primarySpecialty?.name || ''}&keywordId=${primarySpecialty?.id || ''
            ...
            <ConsultantFinderProfileWrapper>
              <MainWrapper>
                <ProfilePageHeader
                  ...
                  topSpecialty={primarySpecialty?.name || ''}
            ...
            <Navigation showOnMobile={true} hasCustomBtnMobile={true}>
              ...
              {serverSideData?.IsLiveDiaryConsultant && (
                ...
                >
                  <Link
                    href={`/finder/step-terms-and-conditions?slug=${serverSideData?.ProfileJson?.slug}&name=${encodeURIComponent(shortName)}&gmcNumber=${gmcNumber}&search=${primarySpecialty?.name || ''}&keywordId=${primarySpecialty?.id || ''}`}
                  >
                    <span>
                      {props.fields.BookOnlineButtonLink.value.text ||
                        'Book online'}

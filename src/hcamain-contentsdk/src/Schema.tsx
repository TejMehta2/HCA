...
import { FooterProps } from 'components/Navigation/Footer/Footer.types';
import { HeaderWithImageProps } from 'components/Page Content/HeaderWithImage/HeaderWithImage';
import { HeroBannerWithSearchProps } from 'components/Page Content/HeroBannerWithSearch/HeroBannerWithSearch';
import { HeroLocationDetailsProps } from 'components/Page Content/HeroLocationDetails/HeroLocationDetails';
import { IntroBlockProps } from 'components/Page Content/IntroBlock/IntroBlock';
import { ContentCardsProps } from 'components/Page Content/ContentCards/ContentCards';
...
// This component should be rendered from the layout, so it can consume layoutData
...
const getSchemaData = (props: SchemaProps) => {
  try {
    ...
    ) as unknown as HeaderWithImageProps | HeroBannerWithSearchProps;
    const locationHeroComponent = components?.find(({ componentName }) =>
      ['HeroLocationDetails'].includes(componentName)
    ) as unknown as HeroLocationDetailsProps;

    const templateId = normalizeId(route?.templateId)
    ...
    const reviewFields = (reviewComponent as unknown as IntroBlockProps)
      ?.fields;
    const reviewFieldsFooter = (footer as unknown as FooterProps)?.fields;
    ...
    if (pageType === 'Hospital/Facility') {
      ...
      ) as unknown as ContentCardsProps;
    ...
    return { noSchema, schemas, curatedLdJsonScripts };
  } catch (err) {
...
const Schema = (props: SchemaProps) => {
  ...
  const { noSchema, schemas, curatedLdJsonScripts } = schemaData;
